...
class ParserDispatcherTest < Test::Unit::TestCase
  ...
  def setup
    parserdispatcher = ParserDispatcher.new("./showcases/webserver.html")
    @script = parserdispatcher.parse
  end
  
  def test_cmdcount
    #TODO: Write test
    assert_equal(21, @script.get_number_commands())
  ...
  def test_beginfile10
    assert_instance_of(RootFile, @script.get_command_at(11))
    bf = @script.get_command_at(11)
  ...
  def test_changefilelines1
    assert_instance_of(ChangeFileLines, @script.get_command_at(14))
    cfl = @script.get_command_at(14)
  ...
  def test_beginfile13
    assert_instance_of(RootFile, @script.get_command_at(15))
    bf = @script.get_command_at(15)
  ...
  def test_declarevariables2
    ...
    assert_equal(4, dv.get_variables().size)
    assert(dv.get_variables[0].include?('username'))
    assert(dv.get_variables[1].include?('password2'))
    assert(dv.get_variables[2].include?('name'))
    assert(dv.get_variables[3].include?('var3'))
  end
  
  # see line 174 - 179
  def test_continuefile1
    assert_instance_of(ContinueFile, @script.get_command_at(20))
    cf = @script.get_command_at(20)
