...
class AddfilecontentParsertest < Test::Unit::TestCase
  ...
  def test_class
    assert_instance_of(AddFileContent, @manual.get_command_at(0))
  end
  
  def test_addfilecontent1
    cmd = @manual.get_command_at(0)
    assert_equal("testfolder/testfile.rb", cmd.get_filename)
    assert_equal("0", cmd.get_line)
    assert_equal("afctest", cmd.get_id)
