...
class ChangefilelinesParsertest < Test::Unit::TestCase
  def setup
    ...
    @manual = @parserdispatcher.parse('showcases/testshowcases/pos/howto_changefilelines.html')
  end
  ...
  def test_class
    assert_instance_of(ChangeFileLines, @manual.get_command_at(0))
  end
  
  def test_changefilelines1
    cmd = @manual.get_command_at(0)
    assert_equal("testfolder/testfile.rb", cmd.get_filename)
    assert_equal("0", cmd.get_startline)
    assert_equal("1", cmd.get_endline)
    assert_equal("cfltest", cmd.get_id)
